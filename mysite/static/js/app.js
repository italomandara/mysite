(function() {
	...
	var displayLoading = function(_where_to, $this) {
		if (_where_to = _inside_element) {
			return $this.css('position', 'relative').append('<div class="loader"></div>');
		} else {
			return $('body').append('<div class="loader-fixed"></div>');
		}
	};

	$.fn.displayLoading = function(_where_to) {
		return displayLoading(_where_to, this);
	};
		...
		$.ajax({
			...
			beforeSend: function() {
				if (typeof $loader !== typeof undefined && $loader) {
					$loader.displayLoading(_inside_element);
				}
			}
		}).done(function(data) {
